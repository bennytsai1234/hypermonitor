...
export function renderChart(canvas, historyData, currentAsset, allData, selectedRange) {
        ...
        try {
            ...
            if (latestTs > lastTs + 1000) {
                ...
                if (currentAsset === 'all') {
                    ...
                    newPoint = {
                        timestamp: allData.timestamp,
                        smart_long_vol_num: toNum(allData.smart_long_vol_num ?? allData.smartLongVolNum),
                        smart_short_vol_num: toNum(allData.smart_short_vol_num ?? allData.smartShortVolNum)
                    };
                ...
                } else if (currentAsset === 'grinder') {
                    newPoint = {
                        timestamp: allData.timestamp,
                        grinder_long_vol_num: toNum(allData.grinder_long_vol_num),
                        grinder_short_vol_num: toNum(allData.grinder_short_vol_num)
                    };
                } else if (currentAsset === 'humble') {
                    newPoint = {
                        timestamp: allData.timestamp,
                        humble_long_vol_num: toNum(allData.humble_long_vol_num),
                        humble_short_vol_num: toNum(allData.humble_short_vol_num)
                    };
                } else if (currentAsset === 'exitLiq') {
                    newPoint = {
                        timestamp: allData.timestamp,
                        exit_liq_long_vol_num: toNum(allData.exit_liq_long_vol_num),
                        exit_liq_short_vol_num: toNum(allData.exit_liq_short_vol_num)
                    };
                } else if (currentAsset === 'semiRekt') {
                    newPoint = {
                        timestamp: allData.timestamp,
                        semi_rekt_long_vol_num: toNum(allData.semi_rekt_long_vol_num),
                        semi_rekt_short_vol_num: toNum(allData.semi_rekt_short_vol_num)
                    };
                } else if (currentAsset === 'fullRekt') {
                    newPoint = {
                        timestamp: allData.timestamp,
                        full_rekt_long_vol_num: toNum(allData.full_rekt_long_vol_num),
                        full_rekt_short_vol_num: toNum(allData.full_rekt_short_vol_num)
                    };
                } else if (currentAsset === 'gigaRekt') {
                    newPoint = {
                        timestamp: allData.timestamp,
                        giga_rekt_long_vol_num: toNum(allData.giga_rekt_long_vol_num),
                        giga_rekt_short_vol_num: toNum(allData.giga_rekt_short_vol_num)
                    };
                }
                ...
                if (newPoint) {
                    ...
                    const parsedData = extractData(allData, currentAsset);
                    const isZero = (toNum(parsedData?.long) === 0 && toNum(parsedData?.short) === 0);
    ...
    const parsedData = extractData(allData, currentAsset);
    const bearish = parsedData ? isBearish(parsedData.sentiment) : false;
    const chartPoints = [];
    ...
    history.forEach((item) => {
        ...
        if (currentAsset === 'all') {
            ...
            l = toNum(item.grinder_long_vol_num);
            s = toNum(item.grinder_short_vol_num);
        } else if (currentAsset === 'humble') {
            l = toNum(item.humble_long_vol_num);
            s = toNum(item.humble_short_vol_num);
        } else if (currentAsset === 'exitLiq') {
            l = toNum(item.exit_liq_long_vol_num);
            s = toNum(item.exit_liq_short_vol_num);
        } else if (currentAsset === 'semiRekt') {
            l = toNum(item.semi_rekt_long_vol_num);
            s = toNum(item.semi_rekt_short_vol_num);
        } else if (currentAsset === 'fullRekt') {
            l = toNum(item.full_rekt_long_vol_num);
            s = toNum(item.full_rekt_short_vol_num);
        } else if (currentAsset === 'gigaRekt') {
            l = toNum(item.giga_rekt_long_vol_num);
            s = toNum(item.giga_rekt_short_vol_num);
        }

        // --- Skip uninitialized historical data (e.g. newly added columns drop to 0) ---
        if (l === 0 && s === 0) return;
