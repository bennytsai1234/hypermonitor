...
class _DashboardScreenState extends State<DashboardScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadHistory() async {
    try {
      ...
      final String? raw = prefs.getString('hyper_history');
      if (raw != null) {
        final List<dynamic> list = jsonDecode(raw);
        setState(() {
          _history.clear();
          _history.addAll(list.map((e) => HyperData.fromJson(e)).toList());
  ...
  Future<void> _saveHistory() async {
    try {
      ...
      await prefs.setString('hyper_history', encoded);
    } catch (e) {
  ...
  void _handleNewData(HyperData newData) {
    ...
    setState(() {
      ...
      _saveHistory(); // Auto-save on update
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_currentData == null && _history.isEmpty) {
      return Scaffold(
    ...
    // Use current data if available, otherwise use last from history
    final data = _currentData ?? _history.last;
    final bool isBearish = data.sentiment.contains("跌");
    final Color sentimentColor = isBearish ? textRed : textGreen;
    final btcNet = (isBearish ? -1 : 1) * ((data.btc?.longVol ?? 0) - (data.btc?.shortVol ?? 0));
    final ethNet = (isBearish ? -1 : 1) * ((data.eth?.longVol ?? 0) - (data.eth?.shortVol ?? 0));
          ...
          SafeArea(
            child: ListView(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(child: MetricCard(label: isBearish ? "BTC 空單" : "BTC 多單", value: isBearish ? (data.btc?.shortDisplay ?? "---") : (data.btc?.longDisplay ?? "---"), delta: null, color: sentimentColor, cardBg: cardBg)),
                ...
                TugOfWarBar(label: "BTC 多空拉鋸", leftVal: data.btc?.longVol ?? 0, rightVal: data.btc?.shortVol ?? 0, leftColor: textGreen, rightColor: textRed, leftLabel: "多", rightLabel: "空", cardBg: cardBg),
                ...
                Row(
                  children: [
                    Expanded(child: MetricCard(label: isBearish ? "ETH 空單" : "ETH 多單", value: isBearish ? (data.eth?.shortDisplay ?? "---") : (data.eth?.longDisplay ?? "---"), delta: null, color: sentimentColor, cardBg: cardBg)),
                    const SizedBox(width: 12),
                ...
                const SizedBox(height: 12),
                TugOfWarBar(label: "ETH 多空拉鋸", leftVal: data.eth?.longVol ?? 0, rightVal: data.eth?.shortVol ?? 0, leftColor: textGreen, rightColor: textRed, leftLabel: "多", rightLabel: "空", cardBg: cardBg),
